  ...
  async submitRemote(run: RunResult): Promise<number | null> {
    ...
    try {
      const res = await fetch(`${r.url}/rest/v1/${r.table ?? 'scores'}`, {
        ...
        body: JSON.stringify({ name: run.name, hero: run.hero, rank: run.rank, seconds: Math.round(run.seconds), hearts_lost: run.heartsLost, accuracy: Math.round(run.accuracy), tasks: run.tasks, score: run.score, version: '1.3.0' }),
      });
